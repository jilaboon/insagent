    ...
    try {
      ...
      for (let f = 0; f < files.length; f++) {
        ...
        for (let c = 0; c < totalChunks; c++) {
          ...
          const percent =
            Math.round((fileProgress + chunkProgress / files.length) * 90) + 10;
          setProgressPercent(percent);
          setStatusMessage(
            `${files[f].name}: מעבד ${(
              (c + 1) * CHUNK_SIZE > rows.length
                ? rows.length
                : (c + 1) * CHUNK_SIZE
            ).toLocaleString("he-IL")} מתוך ${rows.length.toLocaleString(
              "he-IL"
            )} שורות` +
              (totalCreated + totalUpdated > 0
                ? ` (${totalCreated} חדשים, ${totalUpdated} עודכנו)`
                : "")
          );
      ...
      if (abortRef.current) {
        if (currentJobId) {
          await fetch(`/api/import/${currentJobId}/complete`, {
            method: "POST",
          });
        }
